...
const buildRequest = (email, validatedConfig) => {
  ...
  return {
    method: 'POST',
    url: `https://${
      validatedConfig.mailChimpRegion
    }.api.mailchimp.com/3.0/lists/${validatedConfig.mailChimpListID}/members`,
    body: subscriber,
    headers: {
...
exports.handler = (event, context, callback) => {
  ...
  if (validatedBody.error) {
    const first = validatedBody.error.details[0];

    console.log(first.message);
    respond(400, {
      status: first.message,
      details: validatedBody.error.details,
    });
    return;
  }
